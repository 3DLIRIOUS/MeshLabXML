...
def default_topology():
    return {'manifold': True, 'non_manifold_E': 0, 'non_manifold_V': 0}


def parse_topology(ml_log, log=None, print_output=False):
...
def default_hausdorff_distance():
...
def parse_hausdorff(ml_log, log=None, print_output=False):
    ...
    """
    hausdorff_distance = default_hausdorff_distance()
    with open(ml_log) as fread:
